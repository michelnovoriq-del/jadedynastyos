...
export default function DashboardPage() {
    ...
    const copyWebhook = () => {
        if (!metrics) return;
        navigator.clipboard.writeText(`${LIVE_ENGINE_URL}/api/webhooks/stripe/${metrics.organizationId}`);
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <aside className="lg:col-span-4 space-y-6">
                        ...
                        <div className="bg-jade-deep/50 border border-jade-line p-6 relative">
                            ...
                            {!metrics.hasStripeKey ? (
                                <form onSubmit={handleConnectStripe} className="space-y-4">
                                    <input 
                                        ...
                                        required 
                                    />
                                    <button type="submit" className="w-full border border-jade-muted text-jade-muted hover:bg-jade-muted hover:text-black py-2.5 text-[10px] tracking-[0.3em] font-bold uppercase transition-all">
                                        LINK_ENCRYPTION_KEY
                                    </button>
                                </form>
                            ) : (
                            ...
                            {keyStatus && <div className="mt-4 text-[9px] text-jade-muted italic uppercase tracking-widest">{keyStatus}</div>}
                        </div>
